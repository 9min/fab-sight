...
대시보드 필터, 선택 상태, 비교 모드 등 UI 글로벌 상태를 관리한다.
...
```ts
// src/stores/useDashboardStore.ts 예시
...
Lot 데이터, Golden Lot 데이터 등 서버에서 페칭하는 데이터를 관리한다.

```ts
// Query Key 컨벤션
["equipment"]                               // 장비 목록
["equipment", equipmentId, "chambers"]      // 장비의 챔버 목록
["lots", { chamberId }]                     // 챔버별 Lot 목록
["lot", lotId]                              // 특정 Lot 상세 (Wafer 목록 포함)
["lot", lotId, "wafer", waferId]            // 특정 Wafer의 공정 데이터
["golden-lot", { recipeId }]                // 레시피별 Golden Lot 데이터
["recipe", recipeId, "steps"]               // 레시피 스텝 목록
```
