...
│   ├── app/                    # 앱 진입점 (App.tsx)
│   ├── components/             # 컴포넌트
│   │   ├── ui/                 # 공통 UI 컴포넌트 (LotSelector, SensorCheckbox, AnomalyToggle, ToggleButton 등)
│   │   ├── layout/             # 레이아웃 컴포넌트 (TopNav, Sidebar, MainContent, DashboardLayout)
│   │   ├── charts/             # ECharts 차트 컴포넌트 (TimeSeriesChart, RadarChart, LotTrendChart, buildChartOption 등)
│   │   ├── drilldown/          # 드릴다운 패널 컴포넌트 (DrilldownPanel, ParameterTable)
│   │   ├── r2r/                # R2R/APC 패널 컴포넌트 (R2RAdjustmentPanel, ProcessResultTable)
│   │   ├── glossary/           # 용어사전 컴포넌트 (GlossaryButton, GlossaryDrawer, GlossaryCategory)
│   │   └── icons/              # SVG 아이콘 컴포넌트
│   ├── hooks/                  # 커스텀 훅
│   ├── pages/                  # 페이지 컴포넌트 (DashboardPage, LotTrendPage)
│   ├── lib/                    # 외부 라이브러리 설정 (Supabase, QueryClient 등)
│   ├── mocks/                  # Mock 데이터 (MVP 단계, 추후 Supabase 전환)
│   ├── services/               # 비즈니스 로직 및 Supabase 쿼리 함수
│   ├── stores/                 # Zustand 상태 관리 (상세: state-management.md 참조)
│   ├── types/                  # TypeScript 타입 정의
│   ├── utils/                  # 유틸리티 함수 (다운샘플링, 정규화 등)
│   ├── constants/              # 상수 정의
│   └── test/                   # 테스트 설정 파일 (setup.ts, helpers.ts)
├── supabase/
...
├── index.html
├── biome.json                  # Biome 설정
├── lefthook.yml                # Git 훅 설정 (pre-commit 린트/테스트)
├── package.json
├── tsconfig.json
├── vite.config.ts
