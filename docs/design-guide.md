...
데이터 분석 대시보드에 적합한 다크 테마 기반 색상을 사용한다.

| 용도 | 색상 | 설명 |
|------|------|------|
| 배경 (기본) | `slate-900` | 메인 대시보드 배경 |
| 배경 (카드/패널) | `slate-800` | 차트 영역, 사이드바 |
| 배경 (호버) | `slate-700` | 인터랙티브 요소 호버 |
| 텍스트 (기본) | `slate-100` | 주요 텍스트 |
| 텍스트 (보조) | `slate-400` | 레이블, 부가 정보 |
| 테두리 | `slate-700` | 패널 구분선 |
